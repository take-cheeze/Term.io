...
function TerminalSession(command, commandArgs, termSessions, data){
	if ( this instanceof TerminalSession ) {
        ...
        this.termProcess.on('exit', function() {
			self.disconnectAll();
            self.deleteSession();
        });
...
TerminalSession.prototype = {
	...
	newClient: function(client){
        ...
        client.on('disconnect', function() {
			          console.log(self.id + ': connection closed ('+self.clients.length+' connected)');
                  });
	},
    ...
    deleteSession: function() {
		...
		process.nextTick(function() {
			...
			if(self.termSessions){
				delete self.termSessions[self.id];
			}
		});
    },
	clientDisconnect: function(client){
		this.clients = _(this.clients).without(client);
		if(this.clients.length === 0 && !global.config.save_session){
            deleteSession();
		}
	},
